...
<template>
  <section class="screen scene point-exchange-screen">
    <img class="point-title" :src="titleBanner" alt="แลก Point เป็นสิทธิ์เล่น อัตราแลก 10 Point = 1 สิทธิ์ แลกสูงสุดครั้งละ 100 Point (10 สิทธิ์)" />

    <div class="point-panel">
      <img class="point-panel-bg" :src="panelBg" alt="" />
      <span class="panel-balance-num">{{ points.toLocaleString() }}</span>

      <button class="img-btn stepper-btn-img stepper-minus" :disabled="amount <= RATE" @click="step(-RATE)">
        <img :src="btnMinus" alt="ลด" />
      </button>
      <span class="panel-big-num">{{ amount }}</span>
      <button class="img-btn stepper-btn-img stepper-plus" :disabled="amount >= maxAffordable" @click="step(RATE)">
      ...
      <button class="img-btn confirm-btn" :disabled="!canConfirm" @click="openSummary">
        <img :src="btnConfirm" alt="ยืนยันแลก" />
      </button>
    </div>
    <p v-if="preview.bonus" class="rule-note">(รวมโบนัส +{{ preview.bonus }})</p>
    ...
    <div v-if="showSummary" class="modal-backdrop" @click.self="showSummary = false">
      <div class="summary-card">
        <div class="summary-art">
          ...
          <span class="summary-get">{{ preview.total }} <small>สิทธิ์เล่น</small></span>
          <span class="summary-remain">{{ pointsAfter.toLocaleString() }} <small>Point</small></span>

          <div class="summary-actions">
            <button class="img-btn" @click="showSummary = false"><img :src="btnCancel" alt="ยกเลิก" /></button>
            <button class="img-btn" @click="confirm"><img :src="btnConfirmSummary" alt="ยืนยันแลก" /></button>
          </div>
        </div>
      </div>
    </div>
...
<style scoped>
.point-exchange-screen { justify-content: flex-start; }
.point-title { width: min(400px, 90vw); height: auto; margin: 2px 0 16px; }

.rule-note { margin: 8px 0 0; color: #8fbede; font-size: 11.5px; text-align: center; }
.rule-note.warn { color: #ff9aa4; }
...
.point-panel { position: relative; width: min(400px, 94vw); aspect-ratio: 569 / 760;  }
.point-panel-bg { position: absolute; inset: 0; width: 100%; height: 97.5%; object-fit: fill; }
.panel-balance-num {
  position: absolute; left: 60%; width: 32%; top: 10.6%; height: 11.25%;
  display: flex; align-items: center; justify-content: flex-start;
  font-weight: 900; color: #ffd739; font-size:25px;
  text-shadow: 0 2px 4px rgba(0,0,0,.4);
  font-variant-numeric: tabular-nums;
}
.panel-big-num {
  position: absolute; left: 0; width: 100%; top: 18.75%; height: 17.5%;
  display: flex; align-items: center; justify-content: center;
  font-weight: 900; color: #fff; font-size: clamp(36px, 11vw, 52px);
  text-shadow: 0 0 14px rgba(60, 190, 255, .6), 0 3px 4px rgba(0,0,0,.4);
  font-variant-numeric: tabular-nums;
  pointer-events: none;
}
.panel-point-label {
  position: absolute; left: 0; width: 100%; top: 35%; height: 7.5%;
  display: flex; align-items: center; justify-content: center;
  color: #9fd8ff; font-size: 25px; font-weight: 600;
  pointer-events: none;
}
.stepper-btn-img { position: absolute; top: 22.5%; width: 20%; }
.stepper-minus { left: 15%; }
.stepper-plus { right: 15%; }

.quick-picks { position: absolute; left: 15%; width: 70%; top: 45%; display: flex; justify-content: space-between; gap: 8px; }
.quick-pick {
  flex: 1; min-width: 0; padding: 10px 4px; border-radius: 999px; border: 1px solid rgba(101, 197, 255, .5);
  background: linear-gradient(180deg, rgba(20, 60, 110, 0.5), rgba(6, 20, 42, 0.7));
  color: #cfeaff; cursor: pointer; font-weight: 800; font-size: 16px;
}
.quick-pick.active {
  background: linear-gradient(180deg, #2bb9ff, #0872db); border-color: #9fe8ff; color: #fff;
  box-shadow: 0 0 18px rgba(60, 190, 255, .65), inset 0 0 12px rgba(255,255,255,.25);
}

.ticket-chip { position: absolute; left: 27%; width: 45%; top: 56.25%; }
.ticket-chip-bg { width: 100%; height: auto; display: block; }
.ticket-chip-num {
  position: absolute; left: 57%; width: 14%; top: 0; height: 100%;
  display: flex; align-items: center; justify-content: center;
  font-weight: 900; color: #fff; font-size: clamp(16px, 5vw, 22px);
  text-shadow: 0 2px 4px rgba(0,0,0,.4);
}
...
.confirm-btn { position: absolute; left: 11%; width: 78%; top: 68.75%; margin: 0; }
.back-btn { width: min(300px, 80vw);  }
...
.summary-card { width: min(420px, 92vw); display: flex; flex-direction: column; align-items: center; }
.summary-art { position: relative; width: 100%; }
.summary-bg { width: 100%; height: auto; display: block; }
...
.summary-use {
  position: absolute; left: 48%; width: 46%; top: 23.9%; height: 9%;
  display: flex; align-items: center;
  color: #ffd739; font-weight: 900; font-size: clamp(13px, 3.6vw, 18px);
}
.summary-get {
  position: absolute; left: 52.5%; width: 42%; top: 34.4%; height: 9%;
  display: flex; align-items: center;
  color: #ffd739; font-weight: 900; font-size: clamp(13px, 3.6vw, 18px);
}
.summary-remain {
  position: absolute; left: 56%; width: 38%; top: 44.8%; height: 9%;
  display: flex; align-items: center;
  color: #ffd739; font-weight: 900; font-size: clamp(13px, 3.6vw, 18px);
  font-variant-numeric: tabular-nums;
}
.summary-use small, .summary-get small, .summary-remain small {
  color: #eaf6ff; font-weight: 600; margin-left: 5px; font-size: 0.75em;
}
...
.summary-actions { position: absolute; left: 6%; width: 88%; top: 63%; display: flex; gap: 14px; }
.summary-actions .img-btn { flex: 1; min-width: 0; }
</style>
